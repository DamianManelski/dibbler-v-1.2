...
class BulkTest:
   ...
   def __init__(self, name,queryType,requestorPath,arguments):
      ...
      self.IFACE_NAME = 'Połączenie lokalne'
   ...
   def runTest(self):
       ...
       #print(tmpApp, '-i', 'Połączenie lokalne','-dstaddr',self.DST_ADDR, '-bulk', '-m',self.querytype, self.LINK_ADDR);
       if(self.querytype == "ADDRESS"):
          result = subprocess.check_output([tmpApp, '-i', 'Połączenie lokalne','-dstaddr',self.DST_ADDR, '-bulk', '-m',self.querytype, self.LINK_ADDR ])
