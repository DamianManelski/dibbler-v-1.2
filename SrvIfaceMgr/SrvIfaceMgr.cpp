...
SPtr<TSrvMsg> TSrvIfaceMgr::select(unsigned long timeout) {
	...
	if (this->isTcp) {
		...
		case LEASEQUERY_MSG:
		{
							   Log(Debug) << "[Bulk] Leasequery message coming" << LogEnd;
		...
		case RELAY_FORW_MSG:
		{
							   Log(Debug) << "[Bulk] Relay_Forward message coming" << LogEnd;
		...
		case LEASEQUERY_REPLY_MSG:
			Log(Debug) << "[Bulk] Leasequery_Reply message coming" << LogEnd;
